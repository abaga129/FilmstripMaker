...
int main(int argc, const char** argv)
{
	...
	//SetConsoleTitle(TEXT("Filmstrip Maker")); //Windows Only Code. Must Include Windows.h to use.
	...
	for (int i = 0; i < numImages; i++){
		...
		if (img.empty()) //check whether the image is loaded or not
		{
			cout << "Error : Failed to load Image: " << file << endl
			     << "Check spelling and try again..." << endl;
			i = i - 1;
	...
	//-----------Will display the Final image once it is processed
	/*
...
void getImagesAuto(int startingIndex, int pNumImages, string fExt,  vector<Mat> images){
	string fName;
	for(int i = startingIndex; i < pNumImages; i++){
		fname = i + fExt;
		images.push_back(imread(fname, -1);
		                if (img.empty()) //check whether the image is loaded or not
                {
                        cout << "Error : Failed to load Image: " << file << endl
                             << "Check spelling and try again..." << endl;
